...
import com.lolson.encryptsms.contracts.EcSQLiteContract
 ...
 */
class EcKeySvcSQLiteImpl(
    _context: Context
): SQLiteOpenHelper(_context, DB_NAME,null, DB_VERSION), IEcKeySvc {

    // Filename and version
    companion object {
        // If database schema changes, increment version number
        const val DB_NAME = "ec_keys.db"
        ...
        private const val SQL_CREATE_ENTRIES =
            "CREATE TABLE ${EcSQLiteContract.KeyEntry.TABLE_NAME} (" +
                    "${BaseColumns._ID} INTEGER PRIMARY KEY," +
                    "${EcSQLiteContract.KeyEntry.COLUMN_NAME_ID} TEXT," +
                    "${EcSQLiteContract.KeyEntry.COLUMN_NAME_PRIVATEKEY} BLOB," +
                    "${EcSQLiteContract.KeyEntry.COLUMN_NAME_PUBLICKEY} BLOB)"

        /**
         * Delete database
         */
        private const val SQL_DELETE_ENTRIES =
            "DROP TABLE IF EXISTS ${EcSQLiteContract.KeyEntry.TABLE_NAME}"
    }
    ...
    {
        l.d("EKSSLI:: ON_CREATE EC SQLite service implementation")
    ...
    )
    {
        ...
        l.i("EKSSLI:: ON_UPGRADE EC SQLite service implementation from version:" +
                    " $oldVer to: $newVer")
    ...
    {
        l.i("EKSSLI:: ON_DOWNGRADE EC SQLite service implementation from version:" +
                    " $oldVer to: $newVer")
    ...
    {
        l.d("EKSSLI:: GETKEY: EC KEY SQLITE IMPL")
...
//        keyStore.deleteEntry(KEY_STORE_NAME)
        ...
        val projection = arrayOf(
            BaseColumns._ID,
            EcSQLiteContract.KeyEntry.COLUMN_NAME_ID,
            EcSQLiteContract.KeyEntry.COLUMN_NAME_PUBLICKEY,
            EcSQLiteContract.KeyEntry.COLUMN_NAME_PRIVATEKEY)

        val cursor = rdb.query(
            EcSQLiteContract.KeyEntry.TABLE_NAME,     //Database name
...
//        Move read to first position, return false if empty
        if(cursor.moveToFirst() &&
            cursor.getString(1) == EcSQLiteContract.KeyEntry.COLUMN_NAME_ID)
        {
            tempList = KeyPair(
...
//                privateKeyGen(cursor.getBlob(3)))
        ...
        // Print public key
        l.d("EKSSLI:: EC SQLite GET KEY SUCCESS:  ${tempList != null}")
    ...
    ): Boolean
    {
        ...
        val value = ContentValues()
            .apply {
                put(EcSQLiteContract.KeyEntry.COLUMN_NAME_ID,
                    EcSQLiteContract.KeyEntry.COLUMN_NAME_ID)
                put(EcSQLiteContract.KeyEntry.COLUMN_NAME_PUBLICKEY,
                    encrypt(encodePublicKey(key.public)))
                put(EcSQLiteContract.KeyEntry.COLUMN_NAME_PRIVATEKEY,
                    encrypt(encodePrivateKey(key.private)))
            }

        // Insert, returns primary key value: -1 if error
        val newRowId = wdb.insert(EcSQLiteContract.KeyEntry.TABLE_NAME, null, value)
        if (newRowId != -1L)
        {
            result = true
        }
        ...
        // Result
        l.d("EKSSLI:: EC SQLite CREATE result: $result")
        return result
    }
    ...
    ): Boolean
    {
        ...
        val selection = "${EcSQLiteContract.KeyEntry.COLUMN_NAME_ID} LIKE ?"
        val selectArg = arrayOf(EcSQLiteContract.KeyEntry.COLUMN_NAME_ID)

        // Use ID to select row to update
        val count = wdb.update(
            EcSQLiteContract.KeyEntry.TABLE_NAME,
            value,
            selection,
            selectArg)
        ...
        l.d("EKSSLI:: EC SQLITE UPDATE # rows: $count")
        return count > 0
    }
    ...
    {
        return KeyFactory.getInstance("EC").generatePrivate(PKCS8EncodedKeySpec(data))
    }
    ...
    private fun generateStoreKeys()
    {
        ...
        try
        {
            ...
            l.d("EKSSLI:: KEYSTORE KEY GENERATE SUCCESS: ${
        ...
        {
            l.e("EKSSLI:: KEYSTORE KEY GENERATE ERROR: $e")
        }
    ...
    private fun printStoreKeys()
    {
        try
        {
            ...
            {
                l.d("EKSSLI:: KEYSTORE ALIASES: ($con) $ali")
            }
        }
        catch (e: Exception)
        {
            l.e("EKSSLI:: KEYSTORE ALIASES READ ERROR: $e")
        }
    }
    ...
    {
        l.d("EKSSLI:: START ENCRYPT RSA: ${data.size} ${encodeToString(data, DEFAULT).length}")
...
//                ":: ${encodeToString(result, DEFAULT)}")
    ...
    {
        l.d("EKSSLI:: START DECRYPT RSA: ${data.size}  ${encodeToString(data, DEFAULT).length}")
